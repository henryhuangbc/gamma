...
<head>
	...
	<title>HFS %version% %folder%</title>
...
<body>
  ...
  <header class="mdl-layout__header">
    <div class="mdl-layout__header-row">
      <!-- Title -->
      <span class="mdl-layout-title">HFS %version% %folder%</span>
  ...
  <div class="mdl-layout__drawer">
    <span class="mdl-layout-title">Hello, %user%!</span>
    <nav class="mdl-navigation">
	{.$box login.}
  ...
  <main class="mdl-layout__content">
	{.if| {.get|can upload.} |{:
<button id="uploadFAB" class="mdl-button mdl-js-button mdl-button--fab mdl-js-ripple-effect mdl-button--colored">
  <i class="material-icons">cloud_upload</i>
</button>
:}.}
<button id="searchFAB" class="mdl-button mdl-js-button mdl-button--fab mdl-js-ripple-effect mdl-button--colored">
	<i class="material-icons">search</i>
  </button>
			
<!--{.comment|--><h1 style='margin-bottom:100em'>MaterialHFS requires HFS 2.3+ with Macros enabled.</h1> <!--.} -->
<noscript>MaterialHFS requires JavaScript.</noscript>
{.if| {.length|%user%.} |{:
	{.$box panel.}
	{.$box actions.}
	{.$list.}
:}
| Please enter a username (and password if you have an account) by pressing on the hamburger menu, then login.<br><br>If you do not have a user account, do not worry though because you can enter any username to get access to public resources.
.}         
...
<dialog id="uploadDialog" class="mdl-dialog">
  <h3 class="mdl-dialog__title">Upload</h3>
  <div class="mdl-dialog__content">
			{.if| {.get|can upload.} |{:
    		<form action="." method='post' enctype="multipart/form-data" class="mdl-button" style='text-align:right;'>
  ...
  <div class="mdl-dialog__actions">
    <input type='submit' class="mdl-button"></button></form>
...
$(document).ready(function () {
	setTimeout(function() {
    var data = {message: 'Page generated in %build-time%'};
...
function logout() {                               
	...
	var searchDialogOpen = document.querySelector('#searchFAB');
    var searchDialog = document.querySelector('#searchDialog');
    if (! searchDialog.showModal) {
      dialogPolyfill.registerDialog(searchDialog);
    }
    searchDialogOpen.addEventListener('click', function() {
		searchDialog.showModal();
    });
    searchDialog.querySelector('#close')
    .addEventListener('click', function() {
		searchDialog.close();
    });   
	var uploadDialogOpen = document.querySelector('#uploadFAB');
    var uploadDialog = document.querySelector('#uploadDialog');
    if (! uploadDialog.showModal) {
      dialogPolyfill.registerDialog(uploadDialog);
    }
    uploadDialogOpen.addEventListener('click', function() {
       uploadDialog.showModal();
    });
    uploadDialog.querySelector('#close')
    .addEventListener('click', function() {
      uploadDialog.close();
    });                           
...
<div id='files_outer'>
	...
	{.if not| %number% |{: <div style='font-size:200%; padding:1em;'>{.!No {.if|{.length|{.?search.}.}|results|files.}.}</div> :}|{:
        <form method='post'>
            <table id='files' class="mdl-data-table mdl-js-data-table mdl-shadow--2dp"  style="width: 100%;">
...
[box login]		
		{.if| {.length|%user%.} |{:
            {.if|{.can change pwd.} | 
                <a onclick='changePwd.call(this)'  class="mdl-navigation__link">{.!Change password.}</a>
            .}
            :}
...
[box folder]
		{.if| {.length|{.?search.}.} | <a href="." class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored"> {.!BACK.}</a>
		| {.if| {.%folder% != / .} | <a href=".." class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored"> {.!UP.}</a> .}
		/if.}

		{.comment|we need this to start from 1 {.count|folder levels.}.}
		{.breadcrumbs|{:<a  class="mdl-navigation__link" href="%bread-url%" {.if|{.length|%bread-name%.}|.} />
		 {.if|{.length|%bread-name%.}| %bread-name%| {.!Home.}.} /</a>:} .}
...
[box actions]	
		<center>
		{.if|{.can mkdir.}|
		<a id='newfolderBtn' class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored" onclick='ezprompt(this.innerHTML, {type:"text"}, function(s){
				ajax("mkdir", {name:s});
		    });'>{.!New folder.}</a>
		.}
		{.if|{.can comment.}|
		<a id='commentBtn' onclick='setComment.call(this)' class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored">{.!Comment.}</a>
		.}
		{.if|{.get|can delete.}|
		<a id='deleteBtn' onclick='if (confirm("{.!confirm.}")) submit({action:"delete"}, "{.get|url.}")' class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored">{.!Delete.}</a>

		{.if|{.and|{.!option.move.}|{.can move.}.}| <a id='moveBtn' onclick='moveClicked()' class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored">{.!Move.}</a> .}
		.}
		{.if|{.can rename.}|
		<a id='renameBtn' onclick='
            var a = selectedItems();
                if (a.size() != 1)
		    ...
		    });' class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored">{.!Rename.}</a>
		.}
		{.if|{.get|can archive.}|
		<a id='archiveBtn' onclick='if (confirm("{.!confirm.}")) submit({}, "{.get|url|mode=archive|recursive.}")' class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored">{.!Archive.}</a>
		.}
		<a href="{.get|url|tpl=list|sort=|{.if not|{.length|{.?search.}.}|{:folders-filter=\|recursive:}.}.}" class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-button--colored">{.!Get list.}</a>
		</center>
...
[style.css|no log]
body { font-family:Roboto, sans-serif; font-weight:normal; font-size:9pt; background-color:#eef; }
